...
## Release Two — trust and verification console

The existing migration `009-trust.sql`, trust API, credential bidding rules and dispute settlement ledger are now connected to the web console:

- **Trust & verification** opens personal credentials and credentials for managed organizations. Users submit issuer, number, coverage, effective/expiry dates and optional private PDF/image evidence, and withdraw records. Renewal is a new record. Platform administrators have a verification queue with document downloads, a checked-evidence statement and decision notes. Public profiles show verification state, expiry, and what was checked and when.
- Commissioners set credential requirements on open scopes. Bid rows expose expired/missing credentials and link to the bidder's credentials and computed reputation. Existing server rules block expired credentials at bidding and award; current pending submissions satisfy presence requirements and remain visibly unverified. There is no automated issuer/insurer lookup.
- People and organization cards show computed completed-scope ratings and contract value. Detailed reputation names and dates the underlying scopes and shows schedule, quality, communication and closeout scores. The designated payer can publish one immutable review after completion; this existing rule identifies the paying contracting party and does not independently require a bank-confirmed payment.
- **Billing → Reviews & disputes** provides completed-work reviews, scope holds, evidence uploads, notes, split proposals, counterparty acceptance/rejection and withdrawal. Administrators can mediate from the trust queue. Resolution records the split in the existing release/refund ledger; provider failures remain visible as settlement warnings and in Billing.
- A dated JSON evidence packet downloads from the dispute. It includes recorded labor/rates, materials, changes, billing, funding, waiver and evidence metadata. Open packets reflect current records; resolved packets are frozen and hashed. Attached files remain private downloads; the packet is not a ZIP containing those file bytes. Daily reports remain Release Three work.

Validation uses the existing isolated backend suite and a new browser trust workflow, plus billing and console regression tests. Real Stripe sandbox settlement remains the separate outstanding verification described above.
