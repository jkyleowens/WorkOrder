...
export function mountTimeGrid(root, { entries, assignments }, range) {
  ...
  const days = dates.map((date) => {
    ...
    const blocks = entries
      ...
      .map((entry) => {
        const start = end;
        end += Number(entry.hours) * 3600;
        return { entry, start, end };
      });
    return { date, blocks, total: end / 3600 };
  });
  let zoom = 1,
    offset = 0,
    x = 0,
  ...
  function draw() {
    ...
    days.forEach((day, i) => {
      ...
      day.blocks.forEach(({ entry, start, end }, index) => {
        ...
        html += `<button type="button" class="timeline-block tone-${index % 3}" data-entry="${esc(entry.id)}" style="left:${left + 5}px;top:${header + top}px;width:${column - 10}px;height:${bottom - top}px" aria-label="${esc(`${title}, ${entry.hours} hours, ${dateLabel(day.date)}${entry.note ? `, ${entry.note}` : ""}`)}" title="${esc(`${title} · ${entry.hours} h`)}"><strong>${esc(title)}</strong><span>${esc(entry.hours)} h worked</span>${entry.note ? `<small>${esc(entry.note)}</small>` : ""}</button>`;
      });
    });
  ...
  viewport.onclick = (e) => {
    ...
    if (!entry) return;
    detail.hidden = false;
    detail.innerHTML = `<strong>${esc(titles.get(entry.subdivision_id) || `Subdivision ${entry.subdivision_id}`)}</strong><p>${esc(dateLabel(entry.date))} · ${esc(entry.hours)} hours worked</p><p>${esc(entry.note || "No note for this entry.")}</p>`;
  };
